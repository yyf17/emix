...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Mix
        # DUAL TARGET FUNCTIONS 
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
            target_max_qvals1 = self.target_mixer1(target_max_qvals, batch["state"][:, 1:])
            target_max_qvals2 = self.target_mixer2(target_max_qvals, batch["state"][:, 1:])

        min_target_vals = th.min(target_max_qvals1, target_max_qvals2)
        if self.args.mixer == "emix":
            surp_vals = self.supmixer(chosen_action_qvals, batch["state"][:, :-1], th.std(batch["state"][:, :-1], dim=2).unsqueeze(2))
            target_surp_vals = self.target_supmixer(min_target_vals, batch["state"][:, 1:], th.std(batch["state"][:, 1:], dim=2).unsqueeze(2))

        # Calculate 1-step Q-Learning targets
        # TEMPERATURE PARAMETER
        temp = self.args.temp
        targets = rewards + temp*th.logsumexp(target_surp_vals, dim=1) + self.args.gamma * (1 - terminated) * min_target_vals #- 0.25*surprise
        # EMIX OBJECTIVE
        td_error = (temp*th.logsumexp(surp_vals, dim=1) + chosen_action_qvals - targets)
        surprise_vals = th.mean(temp*th.logsumexp(surp_vals, dim=1))
        surprise_ratio = th.mean(temp*th.logsumexp(surp_vals, dim=1) - temp*th.logsumexp(target_surp_vals, dim=1))
